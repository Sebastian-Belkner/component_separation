...
def general_pipeline():
    ...
    path = '/global/homes/s/sebibel/git/component_separation/data/mask/'
...
#     "/global/homes/s/sebibel/git/component_separation/data/
...
#     ['psmaskP_2048.fits.gz', 'gmaskP_apodized_0_2048.fits.gz']
    ...
    spec_filename = 'NPIPE_lmax_{lmax}-lmax_mask_{lmax_mask}-tmask_{tmask}-pmask_{pmask}-freqs_{freqs}_.npy'.format(
        lmax = lmax,
        lmax_mask = lmax_mask,
        tmask = 'lens' if 'lensing' in pmask_filename else 'maude',
        pmask = 'lens' if 'lensing' in pmask_filename else 'maude',
