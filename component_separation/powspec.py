...
class Powspec():
    ...
    def tqupowerspec(tqumap, tmask: List, pmask: List, lmax: int, lmax_mask: int) -> Dict[str, Dict]:
        ...
        def _ud_grade(data, FREQ):
            if int(FREQ)<100:
                return hp.pixelfunc.ud_grade(data, nside_out=self.csu.nside_out[0])
            else:
                return hp.pixelfunc.ud_grade(data, nside_out=self.csu.nside_out[1])

        # retval = np.array(shape=(len(buff.items()),len(self.csu.PLANCKSPECTRUM_f), lmax+1))
        retval = np.array([
            ps.map2cls(
            ...
            ) for FREQC in self.csu.freqcomb 
        ])
        return retval
    ...
    def qupowerspec(qumap: List[Dict[str, Dict]], lmax: int, lmax_mask: int, freqcomb: List[str], specfilter: List[str]) -> Dict[str, Dict]:
        ...
        for FREQC, _ in buff.items():
            spectrum.update({
                FREQC: {
                    spec: buff[FREQC][idx]
                        for idx, spec in enumerate([p for p in self.csu.PLANCKSPECTRUM_f])
                        }
            })
        return spectrum
            ...
            synmap.update({
                freqc: hp.synfast(
                    cls = [
                        ...
                        spectrum[freqc]["BB"],
                        spectrum[freqc]["TE"]],
                    nside = self.csu.nside[0] if int(freqc.split("-")[0])<100 else self.csu.nside[1],
                    new=True)})
        maps = {
    ...
    def build_covmatrices(self, data: np.array, Tscale: str = r"K_CMB"):
        ...
        NFREQUENCIES = len([FREQ for FREQ in self.csu.PLANCKMAPFREQ_f])
        covn = np.zeros(shape=(len(self.csu.PLANCKSPECTRUM), NFREQUENCIES, NFREQUENCIES, lmaxp1))
        for sidx in range(covn.shape[0]):
            for fcombidx, freqc in enumerate(self.csu.freqcomb):
                FREQ1, FREQ2 = int(freqc.split('-')[0]), int(freqc.split('-')[1])
                freqf = np.array([int(n) for n in self.csu.PLANCKMAPFREQ_f])
    ...
    def invert_covmatrices(cov: np.array):
        ...
        for spec in range(len(self.csu.PLANCKSPECTRUM)):
    ...
    def calculate_weights(cov: np.array, Tscale: str = r"K_CMB") -> np.array:
        ...
        """
        # def _elaw(shp):
        #     return np.array([trsf_m.tcmb2trj_sc(FREQ, fr=r'K_CMB', to=Tscale) for FREQ in self.csu.PLANCKMAPFREQ[:-2]])

        elaw = np.array([trsf_m.tcmb2trj_sc(FREQ, fr=r'K_CMB', to=Tscale) for FREQ in self.csu.PLANCKMAPFREQ[:-2]])
